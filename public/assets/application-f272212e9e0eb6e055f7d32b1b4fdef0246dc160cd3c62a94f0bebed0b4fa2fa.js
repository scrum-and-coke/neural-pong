...
$(function() {
  ...
  var Paddle = function(x_pos, controller) {
  ...
  Paddle.prototype.move = function(ball) {
    this.position.y += this.controller(ball, this);
  ...
  var ball = {
    ...
    position: {
      x: cWidth/2 - BALL_DIAMETER/2
      y: cHeight/2 - BALL_DIAMETER/2
    },
    direction: {
      x: 0,
      y: 0
  ...
  function bounceOffPaddle(paddle) {
    return normalize({
      x: -this.direction.x,
      y: this.direction.y + (centre(paddle).y - centre(this).y)/3
    });
  }
  ...
  ball.collide = function(paddleL, paddleR) {
    ...
    if (this.position.y <= 0 || this.position.y >= cHeight)
      ...
      this.direction = bounceOffPaddle(paddleL)
    else if (this.position.x >= paddleR.position.x && within(this, paddleR)) {
      this.direction = bounceOffPaddle(paddleR)
    }
  }
  ...
  var aiController = function(ball, paddle) {
    if (centre(paddle).y < centre(ball).y)
      return 1;
    if (centre(paddle).y > centre(ball).y)
      return -1;
  }

  var leftPaddle = new Paddle(PADDLE_DIST_FROM_SIDE, userController);
  var rightPaddle = new Paddle(cWidth - PADDLE_DIST_FROM_SIDE, aiController);
...
$(function(){
  ...
  Ball.prototype.update = function (paddle1, paddle2) {
      ...
      if(this.direction.x == 0 && this.direction.y == 0 && keysDown[32])
      {
        this.direction.x = 3;
      }
